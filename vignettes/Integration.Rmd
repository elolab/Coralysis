...
Differential gene expression (DGE) between cluster 3 and 5  corresponding roughly to `Visceral endoderm` and `ExE endoderm`, respectively.

```{r dge, fig.width=9, fig.height=4.5}
# DGE analysis: cluster 3 vs 5
dge.clt3vs5 <- FindClusterMarkers(sce,
    clustering.label = "cluster",
    clusters.1 = "3",
    clusters.2 = "5"
)
head(dge.clt3vs5[order(abs(dge.clt3vs5$log2FC), decreasing = TRUE), ])
top6.degs <- head(dge.clt3vs5[order(abs(dge.clt3vs5$log2FC),
    decreasing = TRUE
), "marker"])
