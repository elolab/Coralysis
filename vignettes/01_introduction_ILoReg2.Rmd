...
# Compute PCA & TSNE
sce <- scran::fixedPCA(sce, rank = 30, subset.row = NULL)
set.seed(123)
sce <- RunTSNE(sce)

# Plot TSNE highlighting the batch & cell type
unint.batch.plot <- plotTSNE(sce, colour_by = "batch", point_size = 0.01) + 
    guides(color = guide_legend(override.aes = list(size=2.5))) # to increase the legend dot size
unint.cell.plot <- plotTSNE(sce, colour_by = "cell_type", point_size = 0.01) + 
    guides(color = guide_legend(override.aes = list(size=2.5)))
cowplot::plot_grid(unint.batch.plot, unint.cell.plot, rel_widths = c(0.45, 0.55))
```

Run integration with `ILoReg2`. Provide the batch label column name in `colData(sce)` to the parameter `batch.label`.  

```{r integration, fig.width=7, fig.height=2.5}
# Perform integration with ILoReg2
# start <- Sys.time()
set.seed(1024)
sce <- RunParallelDivisiveICP(object = sce, batch.label = "batch", threads = 4)
# stop <- Sys.time()
# print(stop - start) # Time difference of 11.41677 mins

# Compute PCA with joint cluster probabilities & TSNE
sce <- RunPCA(sce, scale = T)
set.seed(123)
sce <- RunTSNE(sce)

# Plot TSNE highlighting the batch & cell type
int.batch.plot <- plotTSNE(sce, colour_by = "batch", point_size = 0.01) + 
    guides(color = guide_legend(override.aes = list(size=2.5)))
int.cell.plot <- plotTSNE(sce, colour_by = "cell_type", point_size = 0.01) + 
    guides(color = guide_legend(override.aes = list(size=2.5)))
...
# Clustering 
sce <- HierarchicalClustering(object = sce)

# Select no. of clusters
sce <- SelectKClusters(sce, K = 16)

# Plot clustering
clt.plot <- ClusteringScatterPlot(sce, dim.reduction.type = "tsne", return.plot = TRUE, 
...
# Cluster markers 
cluster.markers <- FindAllGeneMarkers(sce)
...
GeneHeatmap(sce, gene.markers = top3.markers)
```
...
dge.clt2vs6 <- FindGeneMarkers(sce, clusters.1 = "2", clusters.2 = "6")
head(dge.clt2vs6)
```
...
ref <- sce[,sce$batch=="V2"] # let V2 assay batch be the reference data set
query <- sce[,sce$batch=="V1"] # let V1 be the query (unknown annotations)
