...
Next, we can examine the gene coefficients from the ICP models that best explain the clusters identified above, allowing us to explore the expression of genes that most strongly support these clusters. Below we will look into the identified similar but unshared cell clusters 5 and 10. 
...
Visualize below the expression for the top 6 positive coefficients for the identified similar but unshared cell populations: clusters 5 and 10.

```{r viz top coefficients}
## Visualize top coefficients for cluster 5, 10 
clts <- c("5", "10")
...
Visualize the expression of the top 6 positive coefficients for cluster 5. The genes _Klk6_ and _Pmp22_ are very specific to cluster 5. Interestingly both genes are are involved in myelination in oligodendrocytes.

```{r viz - cluster 5, fig.width=9.5, fig.height=3.5}
# Cluster 5
...
Visualize the expression of the top 6 positive coefficients for cluster 10. Among the positive coefficients, genes _Slc17a6_ and _Nrn1_ seem quite specific to cluster 10 with _Slc17a6_ known as a marker for glutamatergic neurons. 
...
Overall, ICP gene coefficients supported, to some extent, the identification of similar but unshared cell populations (clusters 5 and 10) with `Coralysis`.
