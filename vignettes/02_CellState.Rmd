...
A PCA and UMAP representation of dataset before performing integration can be obtained with the R package `scran` and `Coralysis`, respectively. Several parameters can be provided to the functions either function in order to adjust the analyses to the dataset (see `?scran::fixedPCA()` and `?Coralysis::RunUMAP`). 
