...
PrepareData.SingleCellExperiment <- function(object) {
    ...
    if (sum(duplicated(rownames(object))) != 0) {
        ...
        message(paste(
            "data in SingleCellExperiment object contained duplicate ",
    ...
    if (is(logcounts(object), "matrix")) {
        ...
        message(paste("Converting object of `matrix` class into `dgCMatrix`.",
            ...
            "increase run time and memory usage drastically!",
            sep = ""
        ))
        ...
        message(paste("Converting object of `data.frame` class into `dgCMatrix`.",
            " Please note that Coralysis has been designed to work with ",
            "sparse data, i.e. data with ",
            "a high proportion of zero values!",
            sep = ""
        ))
        ...
        stop(
            "Error: Data in `logcounts` slot is not of `matrix`, `data.frame` ",
            "or `dgCMatrix` class."
        )
        return(object)
    }
...
#' @aliases PrepareData
setMethod(
    "PrepareData", signature(object = "SingleCellExperiment"),
...
RunPCA.SingleCellExperiment <- function(object, assay.name, p, scale, center, threshold,
    ...
    metadata(object)$coralysis$pca.params <- list(
        "p" = p,
...
#' @aliases RunPCA
setMethod(
    "RunPCA", signature(object = "SingleCellExperiment"),
...
RunUMAP.SingleCellExperiment <- function(object, dims, dimred.type, return.model,
    ...
    if (!(dimred.type %in% reducedDimNames(object))) {
        stop(paste(
            "The 'dimred.type' '", dimred.type, "' is not among 'reducedDimNames(object)':",
...
#' @aliases RunUMAP
setMethod(
    "RunUMAP", signature(object = "SingleCellExperiment"),
...
RunTSNE.SingleCellExperiment <- function(object, dims, dimred.type, perplexity, dimred.name, ...) {
    # Check arguments
    stopifnot(
        is(object, "SingleCellExperiment"), any(is.null(dims), is.numeric(dims)), is.numeric(perplexity),
...
#' @aliases RunTSNE
setMethod(
    "RunTSNE", signature(object = "SingleCellExperiment"),
...
setMethod(
    "FindAllClusterMarkers", signature(object = "SingleCellExperiment"),
...
setMethod(
    "FindClusterMarkers", signature(object = "SingleCellExperiment"),
...
GetCellClusterProbability.SingleCellExperiment <- function(object, icp.run, icp.round, concatenate) {
    ...
    # Check input params
    stopifnot(
        is(object, "SingleCellExperiment"), is(metadata(object)$coralysis$joint.probability, "list"),
...
setMethod(
    "GetCellClusterProbability", signature(object = "SingleCellExperiment"),
...
setMethod(
    "SummariseCellClusterProbability", signature(object = "SingleCellExperiment"),
...
GetFeatureCoefficients.SingleCellExperiment <- function(object, icp.run = NULL, icp.round = NULL) {
    ...
    # Check input
    stopifnot(
        is(object, "SingleCellExperiment"), any(is.null(icp.run), (is.numeric(icp.run) && all(icp.run <= L))),
...
setMethod(
    "GetFeatureCoefficients", signature(object = "SingleCellExperiment"),
...
MajorityVotingFeatures.SingleCellExperiment <- function(object, label) {
    ...
    # Cell clusterings
    cell.clts <- SummariseCellClusterProbability(
        object = object, icp.run = NULL,
    ...
    for (cell in names(clts.label.counts)) {
        ...
        res[[cell]] <- GetFeatureCoefficients(
            object = object, icp.run = out[out$label == cell, "icp_run"],
            icp.round = out[out$label == cell, "icp_round"]
        )[[1]] %>%
...
setMethod(
    "MajorityVotingFeatures", signature(object = "SingleCellExperiment"),
