...
setMethod(
    "PCAElbowPlot", signature(object = "SingleCellExperiment"),
...
setMethod(
    "HeatmapFeatures", signature(object = "SingleCellExperiment"),
...
VlnPlot.SingleCellExperiment <- function(object, clustering.label, features, return.plot, rotate.x.axis.labels) {
    ...
    if (rotate.x.axis.labels) {
        plotlist <- lapply(features, function(x) {
            ggplot(df[df$feature == x, ], aes_string(x = "Cluster", y = "Expression", fill = "Cluster")) +
...
#' @aliases VlnPlot
setMethod(
    "VlnPlot", signature(object = "SingleCellExperiment"),
...
PlotDimRed.SingleCellExperiment <- function(object, color.by, dimred, dims, use.color,
    ...
    p <- ggplot(data = data.plot, mapping = aes(x = .data[[axes[1]]], y = .data[[axes[2]]], color = .data[[color.by]])) +
        ...
        guides(color = guide_legend(
            title = legend.title, nrow = legend.nrow, bycol = TRUE,
...
#' @aliases PlotDimRed
setMethod(
    "PlotDimRed", signature(object = "SingleCellExperiment"),
...
setMethod(
    "PlotExpression", signature(object = "SingleCellExperiment"),
...
PlotClusterTree.SingleCellExperiment <- function(object, icp.run, color.by, use.color,
    ...
    cell.clusters <- SummariseCellClusterProbability(
        object = object, icp.run = icp.run,
    ...
    if (is.null(color.by)) { # if 'color.by' is NULL, plot median cluster probability tree
        ...
        p <- ggplot(data = df) +
            ...
            scatterpie::geom_scatterpie(
                data = df, mapping = aes(x = start, y = as.numeric(Kannot), group = k),
...
setMethod(
    "PlotClusterTree", signature(object = "SingleCellExperiment"),
