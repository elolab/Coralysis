...
BinCellClusterProbability.SingleCellExperiment <- function(object, label, icp.run, icp.round,
    ...
    # Getting probability
    probs <- SummariseCellClusterProbability(
        object = object, icp.run = icp.run, icp.round = icp.round,
    ...
    bins.by.label <- data.bins %>%
        ...
        mutate(
            "aggregated_probability_bins" = get(aggregate.bins.by)(.data$probability),
            "aggregated_label_bins" = paste(.data$label, paste0("bin", .data$probability_bins), sep = "_")
        ) %>%
        ungroup()
    # Feature expression averaged by 'label' x bins
    gexp.bins <- AggregateClusterExpression(
        mtx = assay(x = object, i = use.assay),
...
setMethod(
    "BinCellClusterProbability", signature(object = "SingleCellExperiment"),
...
setMethod(
    "CellClusterProbabilityDistribution", signature(object = "SingleCellExperiment"),
...
TabulateCellBinsByGroup.SingleCellExperiment <- function(object, group, relative, margin) {
    # Check input
    stopifnot(
        is(object, "SingleCellExperiment"), is(metadata(object)$coralysis, "data.frame"),
...
setMethod(
    "TabulateCellBinsByGroup", signature(object = "SingleCellExperiment"),
...
setMethod(
    "CellBinsFeatureCorrelation", signature(object = "SingleCellExperiment"),
